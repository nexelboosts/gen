...
class Agent:
    ...
    def load_config(self):
        try:
            # Load knowledgebase
            kb_path = os.path.join(os.path.dirname(__file__), 'utils', 'knowledgebase.json')
    ...
    def _save_knowledgebase(self):
        try:
            kb_path = os.path.join(os.path.dirname(__file__), 'utils', 'knowledgebase.json')
            os.makedirs(os.path.dirname(kb_path), exist_ok=True)
            with open(kb_path, 'w', encoding='utf-8') as f:
                json.dump(self.knowledgebase, f, ensure_ascii=False, indent=2)
        except Exception:
            pass

    def solve_text_only(self, question_text: str) -> Optional[str]:
        # Check knowledgebase first
        if question_text in self.knowledgebase:
            return self.knowledgebase[question_text]

        try:
            if not self.client:
                return None
            system_prompt = """You are answering a Dutch accessibility hCaptcha challenge.
Strictly follow these rules:
1. Only answer 'ja' if it is correct, 'nee' if it is wrong.
2. Do NOT add anything else — absolutely no punctuation, no spaces, no explanation, only 'ja' or 'nee'."""
            ...
            try:
                ...
                if raw_answer:
                    german_answer = self.convert_to_german(raw_answer)
                    # Save to knowledgebase
                    self.knowledgebase[question_text] = german_answer
                    self._save_knowledgebase()
    ...
    def convert_to_german(self, response: str) -> str:
        ...
        if "ja" in clean or "yes" in clean or clean == "true" or clean.startswith("t") or "ใช่" in clean:
            return "ja"
        # Check for negative responses
        elif "nee" in clean or "no" in clean or clean == "false" or clean.startswith("f") or "ไม่" in clean:
            return "nee"
        return "nee"
...
class Solver:
        ...
        try:
            ...
            if os.path.exists(config_path):
                with open(config_path, 'r') as f:
                    config = json.load(f)
                self.use_ai_solver = config.get('use_ai_solver', True)
        except Exception:
            self.use_ai_solver = True
    ...
    async def change_language_to_dutch(self, page, frame):
        try:
    ...
    async def skip_image_challenge(self, page, frame):
        try:
            language_changed = await self.change_language_to_dutch(page, frame)
            if language_changed:
                await self.human_like_delay(0.1, 0.2)
    ...
    async def solve_captcha(self, page):
        try:
            ...
            if self.use_ai_solver:
                if not self.initialize_ai_agent():
                    # If AI is enabled but fails to init, we might want to fail or fall back.
                    # For now, let's log/return error as before if it was expected to work.
                    self.cleanup_files()
